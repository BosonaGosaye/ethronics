...
import RegisterHeroEditor from '../components/editors/RegisterHeroEditor';
import RegisterObjectiveEditor from '../components/editors/RegisterObjectiveEditor';
import RegisterHighlightsEditor from '../components/editors/RegisterHighlightsEditor';
import RegisterFAQEditor from '../components/editors/RegisterFAQEditor';
import RegisterFormEditor from '../components/editors/RegisterFormEditor';
import RegisterGalleryEditor from '../components/editors/RegisterGalleryEditor';
...
export default function RegisterContentEditor() {
  ...
  const fetchAllLanguages = async () => {
    try {
      ...
      for (const lang of LANGUAGES) {
        try {
          const response = await axios.get(`/register/${lang.code}/${urlSection}`);
          // Extract the content object from the response
          contentMap[lang.code] = response.data.data.content || {};
          statusMap[lang.code] = {
  ...
  const togglePublish = async (language) => {
    ...
    try {
      await axios.patch(`/register/${language}/${urlSection}/publish`);
  ...
  const handleSyncImages = async () => {
    ...
    const imageFieldsMap = {
      ...
      form: [],
      faq: [],
      gallery: ['trainingVideos', 'studentProjects'],
      nextSteps: []
    };
    ...
    try {
      const result = await syncImagesToAllLanguages('register', 'en', urlSection, fields);
      return result;
    } catch (error) {
      throw error;
    }
  ...
  const renderEditor = () => {
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <textarea
              ...
              className="w-full h-[600px] font-mono text-sm p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
              spellCheck={false}
            />
          </div>
        </div>
    ...
    // Use specific editor for hero section
    if (urlSection === 'hero') {
      return (
        <div className="space-y-4">
          {/* Language Tabs */}
    ...
    // Use specific editor for objective section
    if (urlSection === 'objective') {
      return (
        <div className="space-y-4">
          {/* Language Tabs */}
          <div className="bg-white rounded-lg shadow-sm p-4">
    ...
    // Use specific editor for highlights section
    if (urlSection === 'highlights') {
      return (
        <div className="space-y-4">
          {/* Language Tabs */}
          <div className="bg-white rounded-lg shadow-sm p-4">
    ...
    // Use specific editor for FAQ section
    if (urlSection === 'faq') {
      return (
        <div className="space-y-4">
          {/* Language Tabs */}
    ...
    // Use specific editor for form section
    if (urlSection === 'form') {
      return (
        <div className="space-y-4">
          {/* Language Tabs */}
    ...
    // Use specific editor for gallery section
    if (urlSection === 'gallery') {
      return (
        <div className="space-y-4">
          {/* Language Tabs */}
          <div className="bg-white rounded-lg shadow-sm p-4">
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        >
          ...
          <span>Back to Register Dashboard</span>
        </button>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            {/* Sync Images Button (only for hero section) */}
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center space-x-2 px-6 py-2 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-lg hover:from-green-600 hover:to-teal-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
