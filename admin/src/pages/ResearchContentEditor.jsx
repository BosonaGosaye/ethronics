...
import ResearchHeroEditor from '../components/editors/ResearchHeroEditor';
...
const SECTIONS = {
  ...
  whateverYouNeed: 'Whatever You Need',
  collaborate: 'Collaborate',
  cta: 'Call to Action'
};

export default function ResearchContentEditor() {
  ...
  const fetchAllLanguages = async () => {
    try {
      ...
      for (const lang of LANGUAGES) {
        try {
          const response = await axios.get(`/research/${lang.code}/${urlSection}`);
          ...
          if (error.response?.status === 404) {
            contentMap[lang.code] = getDefaultContent('research', urlSection);
  ...
  const togglePublish = async (language) => {
    ...
    try {
      await axios.patch(`/research/${language}/${urlSection}/publish`, {
  ...
  const handleSyncImages = async () => {
    ...
    try {
      const result = await syncImagesToAllLanguages('research', 'en', urlSection, fields);
      return result;
    } catch (error) {
      throw error;
    }
  ...
  const renderEditor = () => {
    ...
    switch (urlSection) {
      case 'hero':
        return <ResearchHeroEditor {...editorProps} />;
      default:
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center space-x-2 px-6 py-2 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-lg hover:from-purple-600 hover:to-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
