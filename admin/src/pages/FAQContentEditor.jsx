...
export default function FAQContentEditor() {
  ...
  const fetchAllLanguages = async () => {
    try {
      ...
      for (const lang of LANGUAGES) {
        try {
          const response = await axios.get(`/faq/${lang.code}/${section}`);
  ...
  const togglePublish = async (language) => {
    ...
    try {
      await axios.patch(`/faq/${language}/${section}/publish`);
  ...
  const handleSyncImages = async () => {
    ...
    try {
      const result = await syncImagesToAllLanguages('faq', 'en', section, fields);
      return result;
    } catch (error) {
      throw error;
    }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        >
          ...
          <span>Back to FAQ Dashboard</span>
        </button>
