...
export default function LibraryContentEditor() {
  ...
  const fetchAllLanguages = async () => {
    try {
      ...
      for (const lang of LANGUAGES) {
        try {
          const response = await axios.get(`/library/${lang.code}/${section}`);
  ...
  const togglePublish = async (language) => {
    ...
    try {
      await axios.patch(`/library/${language}/${section}/publish`);
  ...
  const handleSyncImages = async () => {
    ...
    try {
      const result = await syncImagesToAllLanguages('library', 'en', section, fields);
      return result;
    } catch (error) {
      throw error;
    }
